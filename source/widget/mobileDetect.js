define('widget/mobileDetect', function() {
	var MobileDetect = MD = (function(window, undefined) {
		...
		var getBrowser = function() {
			for (var i in userAgents) {
				if (uagent.search(userAgents[i]) > -1)
					return i;
			}
		...
		var getOS = function() {
			for (var i in operatingSystems) {
				if (uagent.search(operatingSystems[i]) > -1)
					return i;
			}
		...
		var getMobileDeviceName = function() {
			for (var i in phoneDevices) {
				if (uagent.search(phoneDevices[i]) > -1)
					return i;
			}
		...
		var getTabletDeviceName = function() {
			for (var i in tabletDevices) {
				if (uagent.search(tabletDevices[i]) > -1)
					return i;
			}
		...
		return {
			...
			init: function() {
				if (navigator && navigator.userAgent)
					uagent = navigator.userAgent; //"Mozilla/5.0 (Linux; U; Android 4.0.4; en-us; DROID RAZR 4G Build/6.7.2-180_DHD-16_M4-31) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.40"; 
			}
		}
